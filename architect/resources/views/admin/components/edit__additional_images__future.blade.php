 ...
 $images = json_decode($project->images, true); // Force json_decode to return an array
        ...
        <div class="section__additionalimage__wrapper">
            @foreach ($images as $image)
                <form action="{{ route('admin.delete__additional__image__future', ['id'=>$project->id, 'image' =>$image]) }}" method="POST" class="standart__additional__block image-block">
                    @csrf
                    @method('DELETE')
            ...
            @for ($i = count($images); $i < 4; $i++)
                <div class="standart__additional__block upload-block">
                    <form action="{{ route('admin.upload__additional_image__future', ['id' => $project->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
